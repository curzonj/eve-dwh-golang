...
func (f *marketDataFetcher) fetchOrders(regionID int32) ([][]esi.GetMarketsRegionIdOrders200Ok, error) {
	firstPage, resp, err := f.clients.EVERetryClient.ESI.MarketApi.GetMarketsRegionIdOrders(nil, "all", regionID, nil)
	if err != nil {
		return nil, err
	}
...
func (f *marketDataFetcher) remainingOrderPages(regionID int32, lastPage int) ([][]esi.GetMarketsRegionIdOrders200Ok, error) {
	...
	for i := 2; i <= lastPage; i++ {
		go func(page int32) {
			...
			list, _, err := f.clients.EVERetryClient.ESI.MarketApi.GetMarketsRegionIdOrders(nil, "all", regionID, params)
